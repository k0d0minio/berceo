# Decisions: cycle-de-garde-et-annulation
...
- D-112 — A republished garde goes through `publishRequest`, so the new request takes the family's profile commune at that moment (D-63), not a copy of the cancelled request's; the two differ only if she moved in between. Build, 2026-09-25 (a spec gap: §4 said « commune » from the old request).
- D-113 — `.github/workflows/gardes-rappel.yml` treats a 404 from an environment as a skipped call with a warning, not a failure: production answers 404 until the batch carrying the route is promoted. Build, 2026-09-25.

*Collision:* the sibling run avis-etoiles (branch `claude/hopeful-wright-505ax0`, defined 16:44 UTC, a minute after this run's D-105 to D-111 were pushed) numbered its own decisions D-105 to D-111 as well. This run's ids stand (first on a branch, and avis-etoiles depends on this run); avis-etoiles renumbers from D-114 before its Build.
