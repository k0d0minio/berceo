...
- [x] Rules module `src/lib/disponibilites/rules.ts` + `format.ts` and their tests
- [x] Data module `src/lib/disponibilites/nights.ts`
- [x] Words `src/content/disponibilites.ts`, the nav label, `disponibilites.test.ts`
- [x] The family-facing block + `/design-system/portail`
- [x] The professional's page, calendar, server action, nav entry, home link, `routing.test.ts`
