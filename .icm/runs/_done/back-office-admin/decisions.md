...
- D-141 — Neon Auth's admin API (`removeUser`, `revokeUserSessions`) needs a Neon Auth admin session, which the founders do not have (their role lives on `users`, D-33); a suspension ends her sessions and a deletion removes her identity with SQL on the `neon_auth` tables of the same database (`session`, `user`, whose `session` and `account` cascade). The session delete is best effort after the commit; the identity delete is inside the deletion's transaction. Build, 2026-09-25.
- D-142 — The reports list shows no cancellation reason: no reason is stored on a booking (cycle-de-garde-et-annulation), so the spec's « reason given if any » has nothing to show; the side and the kind (annulation or absence) stand for it. Spec gap, Build, 2026-09-25.
- D-143 — « Absences signalées »' reader (`reportedAbsences`, `absenceCount`) and its table component are removed with the page they served, which now redirects; `ADMIN_ABSENCES_PATH` stays for the redirect. Build, 2026-09-25.
