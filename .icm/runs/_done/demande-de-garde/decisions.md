...
*Renumbered at the merge of main (verification-back-office, #39, already holds D-50 to D-59): this run's decisions were D-50 to D-58 in its earlier commits and are D-60 to D-68 from here on, in code comments and run files alike.*
...
- D-61 — Professionals are e-mailed at once for an urgent request and by a daily digest at 18:00 Brussels for normal ones. Operator, Define, 2026-09-25.
- D-62 — The digest is scheduled by a GitHub Actions workflow calling both UAT and production, not by Vercel Cron, which runs only on production deployments and so never on the `uat` custom environment where the founders test (D-22). Define, 2026-09-25.
- D-63 — A request's commune is the family's profile commune, copied at publication and read-only on the form. Operator, Define, 2026-09-25.
- D-64 — « Âge du bébé » is a whole number with a unit: 0 to 12 semaines or 1 to 24 mois, the age on the day of publishing. Operator, Define, 2026-09-25.
- D-65 — Start times are half-hours from 18:00 to 23:00; the end is start + 11 hours, never stored. A family holds at most one open request per night. Define's choices, not the sources'; `revise` changes them. Define, 2026-09-25.
- D-66 — The digest claims its requests (sets `digest_sent_at`) before sending, so two calls racing never send one twice; a send that fails is logged and not retried, and those requests stay on the professional's list. Its Resend idempotency key is `digest-<professional id>-<Brussels day>-<hash of the request ids>` rather than the spec's `digest-<professional id>-<day>`: with the spec's key, a second digest the same day (requests published between the 16:00 and 17:00 UTC calls in summer) would be swallowed by Resend while its requests were already marked. A spec gap; Build, 2026-09-25.
- D-67 — Editing a request is its own page, `/espace/famille/demandes/[id]/modifier` (the spec left the route to Build). The start-time list has no default (the family picks one); the age unit defaults to « semaines ». Build, 2026-09-25.
- D-68 — One `CRON_SECRET` for both environments, not one per environment: the same value on every Vercel environment and as the single GitHub Actions secret `CRON_SECRET`, replacing the spec's `CRON_SECRET_UAT` / `CRON_SECRET_PRODUCTION`. A leaked value can only trigger a digest after 18:00 Brussels, which returns counts and puts each request in one digest at most, so the separation bought little for an extra secret to manage. The spec text still names the two secrets; `revise` aligns it if wanted. Operator, 2026-09-25.
