# Decisions: onboarding-professionnelle
...
- D-43 — The storage variables are `DOCUMENTS_S3_ENDPOINT`, `DOCUMENTS_S3_REGION`, `DOCUMENTS_S3_ACCESS_KEY_ID`, `DOCUMENTS_S3_SECRET_ACCESS_KEY` and `DOCUMENTS_BUCKET`, not the spec's `AWS_*`: Vercel reserves the `AWS_*` names. Build; a naming gap in the spec, same behaviour.
- D-44 — Reopening a validated file (a new profession or any document change) sets it back to `brouillon`, so she passes through the steps and step 4 again; resubmitting appends five declarations and sets a new `submitted_at`. While reopened the profile is hidden, as the dialog says. On a validated file, profession, documents and INAMI number are locked until she confirms the dialog. Build; the spec named the observable outcome, not the intermediate state.
- D-45 — The photo is a `professional_documents` row of kind `photo` (one per profile, replaced on upload), not a `photo_file_id` column: one table, one access check, one route for every private file. Build.
- D-46 — (superseded by D-49 at Release) Postcodes came from bpost's list as published in `jief/zipcode-belgium` (with OpenStreetMap points), placed in NGI's 2025 communes, filtered by bpost's province ranges and a 20 km outlier rule. Statbel and bpost refuse or do not answer a build, and NGI's own postcode table is wrong in places. A stray postcode is an extra suggestion in the picker, never a wrong NIS code. Build.
- D-47 — On a submitted file she cannot remove the last file of a required document: she uploads the new one first, so a file under review never becomes incomplete. Build.
- D-48 — A waiting file (`en_attente`, `complement_demande`) keeps a new profession only when the documents on file already meet it; otherwise she reopens the file (the same dialog as a validated one, now shown on any submitted file) and goes through step 3 and step 4 again. A file under review never becomes incomplete. Release, from the code review.
- D-49 — The professional's zone uses the shared commune list main gained with the family's profile (#34, `src/lib/communes/`, bpost's postcodes), not this run's own register, which was dropped; `professional_communes.commune_ins` matches `family_profiles.commune_ins`, and the migration is `0003_onboarding_professionnelle`. Release, at the merge of main.
