...
- `CRON_SECRET` is not set on Vercel (`env.sh audit --changed` → GAPS 1); Release re-asks it as stop class 3. blocked on operator: set one `CRON_SECRET` value on the Vercel project `berceo` for Production and for the `uat` / Preview environments, and the same value as the GitHub repository secret `CRON_SECRET` (D-68).
...
- Do not switch the digest to Vercel Cron: it never runs on the `uat` environment (D-62).
