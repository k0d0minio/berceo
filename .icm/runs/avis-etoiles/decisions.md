...
- D-105 — Criteria: the family rates the professional on Ponctualité, Communication, Soin and Confiance; the professional rates the family on Accueil, Communication, Clarté des consignes and Respect du cadre. Each criterion is 1 to 5 whole stars, and all four are required. Operator, Define, 2026-09-25.
- D-106 — Double-blind: a rating counts in the note only once both sides have rated the garde, or once 14 days have passed since its end. Publication is computed at read time. This overrides the stub's « update on the first rating ». Operator, Define, 2026-09-25.
- D-107 — The rating window runs from the garde becoming terminée to 14 days after its end. There is one invitation e-mail per side and no reminder. A rating cannot be edited or withdrawn by its author. Operator, Define, 2026-09-25.
- D-108 — The family's note and gardes count show on every request card in the professional's list, before she answers, and on her garde page, never with an identity. The founders get a read-only `/admin/avis` in this run. Operator, Define, 2026-09-25.
- D-109 — The note is the mean of every criterion score in the published ratings received, to one decimal with a comma, with a no-note line when there are none. The gardes count is the terminée, non-annulée bookings, rated or not, hidden at zero. Define, 2026-09-25.
- D-110 — Invitations go out from an hourly pass, `/api/cron/avis-invitations`, called on uat and production by a GitHub Actions workflow with `CRON_SECRET`. Each side is sent at most once, recorded in `rating_invitations`. Nothing is sent for a garde whose window already closed, nor for an annulée garde. Define, 2026-09-25.
- D-111 — Each side's home shows their own note and gardes count. Only the aggregate is shown, never a single rating. Define, 2026-09-25.
