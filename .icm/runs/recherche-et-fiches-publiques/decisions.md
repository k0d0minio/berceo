...
- D-130 — The sitemap is rendered on request (`dynamic = "force-dynamic"`) with the validated professionals' list cached for an hour (`unstable_cache`), instead of a page revalidated hourly: a revalidated sitemap is prerendered at build, and the build must not need the database. Build, 2026-09-25, a spec gap flagged for Release.
- D-131 — The search's result URL carries the field's display alongside `?commune=`: `lieu=<postcode|locality>` when a locality was picked (the field shows it again), `saisie=<text>` when a postcode or name was typed (it resolves again on resubmit). Build, 2026-09-25, a spec gap flagged for Release.
