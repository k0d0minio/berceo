...
- ci: GREEN on 85f90c2 (full gate: Vercel preview, 0007 applied at build; Quality (advisory) passing: lint, typecheck, tests)
...
- [x] The migration generated by drizzle-kit, the journal test, a preview applying it with `db:verify` — 0007 on the merged tree; applied and verified on the run branch and by the preview build of 85f90c2 (its `vercel-build` runs `db:migrate && db:verify`); the journal test passes in the quality job.
