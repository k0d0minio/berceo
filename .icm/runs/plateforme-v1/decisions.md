...
- D-22 — ~~Every run merges into the `uat` branch.~~ Every run merges into `main` (estate D39, 2026-09-24: UAT is the Vercel custom environment `uat`, not a branch). `uat.berceo.be` is the founders' test address and the pace is weekly feedback from Alix and Jordane. The first usable version is due there before December 2026.
