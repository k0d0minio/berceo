...
| The data model | [`src/db/schema.ts`](src/db/schema.ts) — Drizzle: `users` (persona, joined to Neon Auth by `auth_user_id`), the `user_consents` ledger, the professional's file (`professional_*`) and `app_settings`; `db` client in `src/db/index.ts` |
...
| The professional's onboarding, documents, communes | [`src/lib/professionnelle/`](src/lib/professionnelle/) (rules), [`src/lib/documents/`](src/lib/documents/) (private bucket, `/api/fichiers/[id]`), [`src/lib/communes/`](src/lib/communes/) (register, regenerated by `scripts/communes/build.ts`); pages in `src/app/(portail)/espace/professionnelle/`; README → The professional's onboarding |
