...
| The data model | [`src/db/schema.ts`](src/db/schema.ts) — Drizzle: `users` (persona, joined to Neon Auth by `auth_user_id`), the `user_consents` ledger, `family_profiles` (commune, private address), the professional's file (`professional_*`, her availability included), `care_requests`, `care_request_applications`, `bookings`, `app_settings` and the append-only `admin_journal`; `db` client in `src/db/index.ts` |
