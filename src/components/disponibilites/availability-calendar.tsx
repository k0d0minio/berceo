...
import type { CalendarWeek } from "@/lib/disponibilites/rules"
 ...
 * marked, `aria-pressed` whether it is selected. The weeks come from the
...
function AvailabilityCalendar({
  ...
  return (
    <form action={submit} aria-busy={pending} className="flex max-w-md flex-col gap-6">
      ...
      <div role="group" aria-label={p.calendrier} className="flex flex-col gap-0.5">
        ...
        {weeks.map((week) => (
          <Fragment key={week.days[0].date}>
            {week.month ? (
              <p className="pt-3 pb-1 font-display text-h3 text-sauge">{monthHeading(week.month)}</p>
            ) : null}
