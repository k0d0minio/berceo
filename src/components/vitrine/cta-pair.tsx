 ...
 * caller's (D-25). Below sm the capsules' side padding drops to the DA's 24 px
 * floor so the longer family label fits a 320 px screen on one line
 * (premier-ecran).
 */
function CtaPair({
  family,
  ...
  return (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:flex-wrap", className)}>
      <Button asChild variant="sauge" className="px-6 sm:px-7">
        <Link href={family.href}>{family.label}</Link>
      </Button>
      <Button asChild variant="blanc" className="px-6 sm:px-7">
