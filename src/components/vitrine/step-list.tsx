 ...
 *
 * From md the steps sit side by side and share three rows (a subgrid): the
 * numbers on one line, the titles on the next, and the paragraphs start
 * together however many lines a title wraps to (blocs-accueil).
 */
function StepList({
  steps,
  className,
  ...
  return (
    <ol className={cn("grid gap-8 md:grid-cols-3 md:gap-y-3", className)}>
      {steps.map((step, index) => (
        <li
          key={step.title}
          className="flex flex-col gap-3 md:row-span-3 md:grid md:grid-rows-subgrid"
        >
          <span
            aria-hidden
